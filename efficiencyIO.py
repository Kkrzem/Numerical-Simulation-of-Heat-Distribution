...
def read_data(file_path):
    ...
    for line in data:
        ...
        elif "Efficiency IO" in line and current_size in sizes:
            try:
                Efficiency_IO = float(line.split('Efficiency IO:')[1].split(',')[0])
                Efficiency_data[current_size].append(Efficiency_IO)
...
def plot_data(Efficiency_data):
    ...
    plt.ylabel('Efficiency IO')
    plt.title('OpenMP Efficiency IO vs. Number of Threads for Different Sizes')
    plt.legend()
    plt.grid(True)
    plt.show()
