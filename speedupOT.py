...
def read_data(file_path):
    ...
    for line in data:
        ...
        elif "Speedup OT" in line and current_size in sizes:
            try:
                speedup_ot = float(line.split('Speedup OT: ')[1].split(',')[0])
...
def plot_data(speedup_data):
    ...
    for i, (size, speedups) in enumerate(speedup_data.items()):
        ...
        #         # Add data labels
        # for x, y in zip(thread_counts, speedups):
    ...
    plt.ylabel('Speedup OT')
    plt.title('OpenMP Speedup OT vs. Number of Threads for Different Sizes')
