...
def read_data(file_path):
    ...
    for line in data:
        if "Running parallel program on size" in line: 
            ...
            try:
                Efficiency_ot = float(line.split('Speedup I/O: ')[1].split(',')[0])
                Efficiency_data[current_size].append(Efficiency_ot)
...
def plot_data(Efficiency_data):
    ...
    plt.ylabel('Efficiency OT')
    plt.title('OpenMP Efficiency OT vs. Number of Threads for Different Sizes')
    plt.legend()
    plt.grid(True)
    plt.show()
